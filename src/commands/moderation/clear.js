...
let color = guildInfo.color
    ...
    if (!message.member.permissions.has('MANAGE_MESSSAGES')) return message.channel.send(noPermissionsEmbed);
    ...
    if (isNaN(argsn[0]) || parseInt(argsn[0]) <= 0) {
      const numberEmbed = new Discord.MessageEmbed()
        .setColor(color)
        .setDescription(`${emojiAttention} ${lang.ClearErrorNumber}`)
        .setTimestamp()
        .setFooter(message.author.username, message.author.avatarURL());
    ...
    if (parseInt(argsn[0]) > 100) {
      const clearEmbed = new Discord.MessageEmbed()
        .setColor(color)
        .setDescription(`${emojiAttention} ${lang.ClearErrorMoreCent}`)
        .setTimestamp()
        .setFooter(message.author.username, message.author.avatarURL());
