...
module.exports = {
    ...
    async execute(message, args, client, lang, guildInfo) {
        ...
        const Nopresencestatutsoutien = new Discord.MessageEmbed()
            .setColor(color)
            .setDescription(`${emojiAttention} ${lang.RoleReactionErrorNoRole}`)
            .setTimestamp()
            .setFooter(`${client.user.username} © 2021`, `${client.user.displayAvatarURL()}`);

        const rolenotfound = new Discord.MessageEmbed()
            .setColor(color)
            .setDescription(`${emojiAttention} <@${message.author.id}> ${lang.RoleReactionErrorRoleNotFound}`)
            .setTimestamp()
            .setFooter(`${client.user.username} © 2021`, `${client.user.displayAvatarURL()}`);

        const rolemanaged = new Discord.MessageEmbed()
            .setColor(color)
            .setDescription(`${emojiAttention} <@${message.author.id}> ${lang.RoleReactionErrorRoleManaged}`)
            .setTimestamp()
            .setFooter(`${client.user.username} © 2021`, `${client.user.displayAvatarURL()}`);

        const roleprblm = new Discord.MessageEmbed()
            .setColor(color)
            .setDescription(`${emojiAttention} <@${message.author.id}> ${lang.RoleReactionErrorRoleHighest}`)
            .setTimestamp()
            .setFooter(`${client.user.username} © 2021`, `${client.user.displayAvatarURL()}`);
        ...
        if (message.author.id === message.guild.owner.id || guildInfo.owner.includes(message.author.id) || message.author.id === owner) {
            ...
            await message.channel.awaitMessages(filter, {
                ...
                .then(async (collected) => {
                    ...
                    const newGuildInfo = await client.db.guilds.findOne({
                        id: message.guild.id
                    })
                    try {
                        msg = await message.guild.channels.cache.get(newGuildInfo.roleMenuChannel).messages.fetch(msgid)
                    } catch (error) {
                        try {
            ...
            await message.channel.awaitMessages(filter, {
                ...
                .then(async (collected) => {
                    ...
                    const actualMessage = await client.db.guilds.findOne({
                        id: message.guild.id
                    });
                    const roleMenu = actualMessage.role_menu.some(roleReact => roleReact.message_id === msgid);
                    if (!roleMenu) {
                        await client.db.guilds.findOneAndUpdate({
                            id: message.guild.id
                        }, {
                        ...
                        const index = actualMessage.role_menu.find(roleReact => roleReact.message_id === msgid);
                        await client.db.guilds.findOneAndUpdate({
                            id: message.guild.id,
                            'role_menu.message_id': msgid
                        }, {
                            $push: {
